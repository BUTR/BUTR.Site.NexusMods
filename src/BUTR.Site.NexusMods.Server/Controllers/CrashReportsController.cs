        ...
        IQueryable<UserCrashReportView> DbQueryBase(Expression<Func<CrashReportEntity, bool>> predicate)
        {
            return _dbContextRead.CrashReports
                ...
                .Select(x => new UserCrashReportView
                {
                    ...
                    TopInvolvedModuleId = x.ModuleInfos.OrderBy(y => y.Index).Where(z => z.IsInvolved).Select(y => y.Module).Select(y => y.ModuleId).Cast<ModuleId?>().FirstOrDefault(),
                    InvolvedModuleIds = x.ModuleInfos.OrderBy(y => y.Index).Where(z => z.IsInvolved).Select(y => y.Module).Select(y => y.ModuleId).ToArray(),
