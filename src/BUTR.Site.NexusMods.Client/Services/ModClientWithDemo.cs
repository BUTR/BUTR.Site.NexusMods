...
public sealed class ModClientWithDemo : IModClient
{
    ...
    public async Task<ModModelPagingDataAPIResponse> ModPaginatedAsync(PaginatedQuery? body, CancellationToken ct)
    {
        ...
        if (token?.Type.Equals("demo", StringComparison.OrdinalIgnoreCase) == true)
        {
            ...
            return new ModModelPagingDataAPIResponse(new ModModelPagingData(mods, new PagingMetadata(1, (int) Math.Ceiling((double) mods.Count / (double) body.PageSize), body.PageSize, mods.Count, 0)), string.Empty);
        }
    ...
    public async Task<ModNexusModsManualLinkModelPagingDataAPIResponse> ModManualLinkPaginatedAsync(PaginatedQuery? body, CancellationToken ct)
    {
        var token = await _tokenContainer.GetTokenAsync(ct);
        if (token?.Type.Equals("demo", StringComparison.OrdinalIgnoreCase) == true)
            return new ModNexusModsManualLinkModelPagingDataAPIResponse(new ModNexusModsManualLinkModelPagingData(new List<ModNexusModsManualLinkModel>(), new PagingMetadata(1, 1, body.PageSize, 1, 0)), string.Empty);
    ...
    public async Task<UserAllowedModsModelPagingDataAPIResponse> AllowUserAModPaginatedAsync(PaginatedQuery? body, CancellationToken ct)
    {
        var token = await _tokenContainer.GetTokenAsync(ct);
        if (token?.Type.Equals("demo", StringComparison.OrdinalIgnoreCase) == true)
            return new UserAllowedModsModelPagingDataAPIResponse(new UserAllowedModsModelPagingData(new List<UserAllowedModsModel>(), new PagingMetadata(1, 1, body.PageSize, 1, 0)), string.Empty);
